...
export function MeetingStage({ 
  ...
  return (
    <div className={cn(
      "flex flex-col h-screen w-full bg-[#0F172A] text-slate-100 font-sans overflow-hidden transition-colors duration-1000",
      isWarning && "border-t-4 border-amber-500",
      isOvertime && "border-t-4 border-red-500/50"
    )}>
      {/* Top Bar */}
      <header className="h-20 border-b border-slate-800 flex items-center justify-between px-4 md:px-8 bg-[#1E293B] shadow-lg shrink-0">
        <div className="flex items-center gap-6">
          <div className="flex flex-col">
            <span className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Relógio</span>
          ...
          <div className="flex flex-col hidden md:flex">
            <span className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Término Previsto</span>
        ...
        <div className="flex items-center gap-4 md:gap-6 text-right">
          <div className="flex flex-col">
            <span className="text-[10px] uppercase tracking-widest text-slate-400 font-bold font-mono">Saldo Total</span>
            <span className={cn(
      ...
      <main className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-0 overflow-hidden">
        <aside className="hidden md:flex col-span-3 border-r border-slate-800 bg-[#0F172A]/50 flex-col overflow-hidden">
          ...
          <div className="flex-1 p-2 space-y-1 overflow-y-auto opacity-90">
            {state.parts.map((part, index) => {
              ...
              if (index < state.currentPartIndex) {
                ...
                statusClass += "bg-[#1E293B] border-[#4A6CA7] rounded ring-1 ring-[#4A6CA7]/30";
              ...
              return (
                <div key={part.id} className={statusClass}>
                  ...
                  {!part.hideSpeaker && (
                    <span className={cn("text-xs", index === state.currentPartIndex ? "text-slate-300" : "text-slate-400")}>
        ...
        <section className={cn(
            ...
            >
              ...
              <div className="relative">
                <div className={cn(
                  ...
                  isOvertime ? "text-red-500" : isWarning ? "text-amber-400" : "text-white"
                )}>
          ...
          <div className="absolute bottom-8 flex gap-4 opacity-30 hover:opacity-100 transition-opacity z-20">
             <button onClick={() => onAdjustTimer(-60)} className="w-10 h-10 md:w-12 md:h-12 bg-slate-900 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 font-mono text-xs md:text-sm border border-slate-800 transition-colors">-1m</button>
             <button onClick={() => onAdjustTimer(-15)} className="w-10 h-10 md:w-12 md:h-12 bg-slate-900 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 font-mono text-xs md:text-sm border border-slate-800 transition-colors">-15s</button>
             <button onClick={() => onAdjustTimer(15)} className="w-10 h-10 md:w-12 md:h-12 bg-slate-900 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 font-mono text-xs md:text-sm border border-slate-800 transition-colors">+15s</button>
             <button onClick={() => onAdjustTimer(60)} className="w-10 h-10 md:w-12 md:h-12 bg-slate-900 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 font-mono text-xs md:text-sm border border-slate-800 transition-colors">+1m</button>
          </div>
        </section>

        <aside className="hidden md:flex col-span-3 border-l border-slate-800 flex-col bg-[#0F172A]/50">
          <div className="p-6 space-y-8 flex-1 flex flex-col">
            <div className="space-y-4">
              <h3 className="text-[11px] uppercase tracking-widest text-slate-500 font-bold border-b border-slate-800 pb-2">Métricas da Parte</h3>
            ...
            {state.currentPartIndex + 1 < state.parts.length && !state.isCounselPhase && (
              <div className="space-y-4">
                <h3 className="text-[11px] uppercase tracking-widest text-slate-500 font-bold border-b border-slate-800 pb-2">Próxima Transição</h3>
                <div className="flex items-center gap-3 bg-slate-800/40 p-3 rounded-xl border border-slate-700">
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    {!state.parts[state.currentPartIndex + 1].hideSpeaker && (
                      <span className="text-[10px] text-slate-400 italic font-mono truncate">
      ...
      <footer className="h-20 md:h-24 bg-[#1E293B] border-t border-slate-800 p-4 flex gap-4 shrink-0 z-50">
        {buttonState === 'start' && (
          <button 
        ...
        {buttonState === 'counsel' && (
          <>
            ...
            <button 
              ...
              className="flex-1 bg-[#0F172A] hover:bg-black transition-colors rounded-xl flex items-center justify-center gap-2 border border-slate-700 text-slate-300"
            >
